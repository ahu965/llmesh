...
class SimulateResponse(BaseModel):
    ...
    pinned_models: List[PinnedSimulateItem] = []  # 任务组 pinned 阶段候选列表（无 pinned 时为空）
...
def _simulate(
    ...
    if thinking is True:
        available = [m for m in available if m["thinking_mode"] in ("optional", "always")]
    ...
    if pinned:
        for idx, item in enumerate(pinned, start=1):
            if isinstance(item, dict):
                vm: str = item.get("vm", "")
                thinking_override: Optional[bool] = item.get("thinking")  # None=继承全局
            else:
                vm = str(item)
            ...
            if m:
                parts = vm.split("/", 1)
                pinned_items.append(PinnedSimulateItem(
                    order=idx,
                    vendor=parts[0],
...
def simulate_meta(session: Session = Depends(get_session)):
    ...
    task_groups = [TaskGroupMeta(name=tg.name, display_name=tg.display_name) for tg in tgs]
